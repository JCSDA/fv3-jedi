...
#include <netcdf.inc>
   ...
   !>@brief The subroutine 'read_grid' reads the grid from the mosaic grid file.
     subroutine read_grid(Atm, grid_file, ndims, nregions, ng)
       ...
       if(len_trim(units) < 6) call mpp_error(FATAL, &
             "fv_grid_tools_mod(read_grid): the length of units must be no less than 6" // units)
   ...
   !    real(kind=R_GRID) :: grid_global(1-ng:npx  +ng,1-ng:npy  +ng,ndims,1:nregions)
       ...
       real(kind=kind_real), pointer, dimension(:,:) :: rarea_u, rarea_v
       real(kind=kind_real), pointer, dimension(:,:) :: rdx6, rdy6

       real(kind=R_GRID), pointer, dimension(:,:) :: sina, cosa, dx, dy, dxc, dyc, dxa, dya
       real(kind=kind_real), pointer, dimension(:,:,:) :: e1, e2

       real(kind=kind_real), pointer, dimension(:,:) :: rarea, rarea_c
       real(kind=kind_real), pointer, dimension(:,:) :: rdx, rdy, rdxc, rdyc, rdxa, rdya
   ...
   ! The following is a hack to get pass the am2 phys init:
          do j=max(1,jsd),min(jed,npy)
             do i=max(1,isd),min(ied,npx)
                grid(i,j,1) = lon_rad - 0.5*domain_rad + real(i-1,kind=R_GRID)/real(npx-1,kind=R_GRID)*domain_rad
                grid(i,j,2) = lat_rad - 0.5*domain_rad + real(j-1,kind=R_GRID)/real(npy-1,kind=R_GRID)*domain_rad
             enddo
          enddo
       ...
       subroutine setup_orthogonal_grid(npx, npy, bd, grid_file)
         ...
         real(kind=kind_real), allocatable, dimension(:,:)  :: tmpx, tmpy, tmpu, tmpv, tmpa
   ...
   !      cosa(:,:) = 0.
       ...
       subroutine setup_aligned_nest(Atm)
         ...
         real(kind=kind_real),    pointer, dimension(:,:,:) :: wt_h, wt_u, wt_v

         integer, pointer, dimension(:,:,:) :: ind_b
         real(kind=kind_real),    pointer, dimension(:,:,:) :: wt_b
   ...
   !!$           (ieg-isg+2)*(jeg-jsg+1)*ndims, mpp_root_pe())
       ...
       subroutine setup_latlon(deglon_start,deglon_stop, deglat_start, deglat_stop, bd )
         ...
         do j=jsd,jed+1
            do i=isd,ied+1
               grid(i,j,1) = lon_start + real(i-1,kind=R_GRID)*dl
               grid(i,j,2) = lat_start + real(j-1,kind=R_GRID)*dp
            enddo
         enddo

...
use fv3jedi_kinds_mod,          only: kind_real
...
#ifdef OVERLOAD_R4
     real(kind=kind_real), parameter:: real_big = 1.e8    ! big enough to cause blowup if used
#else
     real(kind=kind_real), parameter:: real_big = 1.e30   ! big enough to cause blowup if used
#endif

     !This is now exclusively for fields that need to be available outside of fv_diagnostics
     type fv_diag_type

        real(kind=kind_real), allocatable :: zxg(:,:)

        integer :: id_u_dt_sg, id_v_dt_sg, id_t_dt_sg, id_qv_dt_sg
        integer :: id_ws, id_te, id_amdt, id_mdt, id_divg, id_aam
        logical :: initialized = .false.
        real(kind=kind_real)  sphum, liq_wat, ice_wat       ! GFDL physics
        real(kind=kind_real)  rainwat, snowwat, graupel, hailwat

        real(kind=kind_real) :: efx(max_step), efx_sum, efx_nest(max_step), efx_sum_nest, mtq(max_step), mtq_sum
        integer :: steps
   ...
   !! is altered outside of those two modules.
     type fv_grid_type
        ...
        real(kind=kind_real), allocatable, dimension(:,:,:) :: grid, agrid
        real(kind=kind_real), allocatable, dimension(:,:) :: area, area_c
        real(kind=kind_real), allocatable, dimension(:,:) :: rarea, rarea_c

        real(kind=kind_real), allocatable, dimension(:,:) :: sina, cosa
        real(kind=kind_real), allocatable, dimension(:,:,:) :: e1,e2
        real(kind=kind_real), allocatable, dimension(:,:) :: dx, dy
        real(kind=kind_real), allocatable, dimension(:,:) :: dxc, dyc
        real(kind=kind_real), allocatable, dimension(:,:) :: dxa, dya
        real(kind=kind_real), allocatable, dimension(:,:) :: rdx, rdy
        real(kind=kind_real), allocatable, dimension(:,:) :: rdxc, rdyc
        real(kind=kind_real), allocatable, dimension(:,:) :: rdxa, rdya

   !  MOLECULAR_DIFFUSION
        real(kind=R_GRID), allocatable, dimension(:,:) :: area_u_64, area_v_64
        real(kind=R_GRID), allocatable, dimension(:,:) :: dx6_64, dy6_64
        real(kind=kind_real), allocatable, dimension(:,:) ::  area_u,  area_v
        real(kind=kind_real), allocatable, dimension(:,:) :: rarea_u, rarea_v
        real(kind=kind_real), allocatable, dimension(:,:) ::  dx6,  dy6
        real(kind=kind_real), allocatable, dimension(:,:) :: rdx6, rdy6
        real(kind=kind_real), allocatable, dimension(:,:) :: sina_6
        real(kind=kind_real), allocatable, dimension(:,:) :: delu_6, delv_6
        real(kind=kind_real), allocatable, dimension(:,:) :: delu_5, delv_5
        ...
        real(kind=kind_real), allocatable :: l2c_u(:,:), l2c_v(:,:)
        ! divergence Damping:
        real(kind=kind_real), allocatable :: divg_u(:,:), divg_v(:,:)    !
        ! del6 diffusion:
        real(kind=kind_real), allocatable :: del6_u(:,:), del6_v(:,:)    !
        ! Cubed_2_latlon:
        ...
        ! latlon_2_cubed:
        real(kind=kind_real), allocatable :: z11(:,:)
        real(kind=kind_real), allocatable :: z12(:,:)
        real(kind=kind_real), allocatable :: z21(:,:)
        real(kind=kind_real), allocatable :: z22(:,:)

   !    real(kind=kind_real), allocatable :: w00(:,:)

        real(kind=kind_real), allocatable :: cosa_u(:,:)
        real(kind=kind_real), allocatable :: cosa_v(:,:)
        real(kind=kind_real), allocatable :: cosa_s(:,:)
        real(kind=kind_real), allocatable :: sina_u(:,:)
        real(kind=kind_real), allocatable :: sina_v(:,:)
        real(kind=kind_real), allocatable :: rsin_u(:,:)
        real(kind=kind_real), allocatable :: rsin_v(:,:)
        real(kind=kind_real), allocatable ::  rsina(:,:)
        ...
        real(kind=kind_real), allocatable :: eww(:,:)
        real(kind=kind_real), allocatable :: ess(:,:)

        ! Unit vectors for lat-lon grid
        real(kind=R_GRID), allocatable :: vlon(:,:,:), vlat(:,:,:)
        real(kind=kind_real), allocatable :: fC(:,:), f0(:,:)
        ...
        real(kind=kind_real)  :: acapN, acapS
        real(kind=kind_real)  :: globalarea  !< total Global Area
   ...
   !> Tracer transport options:
      ...
      real(kind=kind_real)    :: scale_z = 0.   !< diff_z = scale_z**2 * 0.25 (only used for Riemann solver)
      real(kind=kind_real)    :: w_max = 75.    !< Not used.
      real(kind=kind_real)    :: z_min = 0.05   !< Not used.
      real(kind=kind_real)    :: lim_fac = 1.0  !< linear scheme limiting factor when using hord = 1. 1: hord = 5, 3: hord = 6
      ...
      real(kind=kind_real)    :: dddmp = 0.0    !< Dimensionless coefficient for the second-order Smagorinsky-type
                                !< divergence damping. The default is value is 0.0. 0.2
                                !< (the Smagorinsky constant) is recommended if ICs are noisy.
      real(kind=kind_real)    :: d2_bg = 0.0    !< Coefficient for explicit second-order divergence damping.
      ...
      real(kind=kind_real)    :: d4_bg = 0.16   !< Dimensionless coefficient for explicit higher-order divergence damping.
      ...
      real(kind=kind_real)    :: vtdm4 = 0.0    !< Coefficient for explicit other-variable damping. The value of 'vtdm4'
      ...
      real(kind=kind_real)    :: trdm2 = 0.0    !< Coefficient for del-2 tracer damping
      real(kind=kind_real)    :: d2_bg_k1 = 4.         !< Strength of second-order diffusion in the top sponge layer.
      ...
      real(kind=kind_real)    :: d2_bg_k2 = 2.         !< Strength of second-order diffusion in the second sponge
                                       !< layer from the model top. This value must be specified, and
                                       !< should be less than 'd2_bg_k1'.
      real(kind=kind_real)    :: d2_divg_max_k1 = 0.15 !< d2_divg max value (k=1)
      real(kind=kind_real)    :: d2_divg_max_k2 = 0.08 !< d2_divg max value (k=2)
      real(kind=kind_real)    :: damp_k_k1 = 0.2       !< damp_k value (k=1)
      real(kind=kind_real)    :: damp_k_k2 = 0.12      !< damp_k value (k=2)
   ...
   !> PG off centering:
      real(kind=kind_real)    :: beta  = 0.0  !< Parameter specifying fraction of time-off-centering for backwards
      ...
      real(kind=kind_real)    :: d_ext = 0.   !< Coefficient for external (barotropic) mode damping. The
      ...
      real(kind=kind_real)    :: d_ext = 0.02  !< Coefficient for external (barotropic) mode damping. Proper range is 0 to 0.02.
   ...
   !-----------------------------------------------------------
      real(kind=kind_real) :: shift_fac = 18.   !< Westward zonal rotation (or shift) of cubed-sphere grid from
   ...
   !-----------------------------------------------------------------------------------------------
      ...
      real(kind=kind_real)    :: p_fac = 0.05  !< Safety factor for minimum nonhydrostatic pressures, which
      ...
      real(kind=kind_real)    :: a_imp = 0.75  !< Controls behavior of the non-hydrostatic solver. Values > 0.5
      ...
      real(kind=kind_real)    :: fac_n_spl = 1.0 !< factor multiplying n_split up tp forecast hour fhouri
      real(kind=kind_real)    :: fhouri    = 0.0 !< forecast hour up to which the number of small dynamics (acoustic) time steps
   ...
   !------------------------------------------
      ...
      real(kind=kind_real)    :: sg_cutoff = -1   !< cutoff level for fv_sg_adj (2dz filter; overrides n_sponge)
      ...
      real(kind=kind_real)    :: p_ref = 1.E5   !< Surface pressure used to construct a horizontally-uniform reference
      ...
      real(kind=kind_real)    :: dry_mass = 98290.  !< If adjust_dry_mass is .true., sets the global dry air mass,
      ...
      integer :: nt_prog = 0
      integer :: nt_phys = 0
      real(kind=kind_real)    :: tau_h2o = 0.  !< Time-scale (days) for simple methane chemistry to act as
      ...
      real(kind=kind_real)    :: delt_max = 1.  !< Maximum allowed magnitude of the dissipative heating rate, K/s;
      ...
      real(kind=kind_real)    :: d_con = 0.  !< Fraction of kinetic energy lost to explicit damping to be
      ...
      real(kind=kind_real)    :: ke_bg = 0.  !<  background KE production (m^2/s^3) over a small step
                             !< Use this to conserve total energy if consv_te=0
      real(kind=kind_real)    :: consv_te = 0.   !< Fraction of total energy lost during the adiabatic integration
      ...
      real(kind=kind_real)    :: tau_w = 0.      !< Time scale (in days) for Rayleigh friction applied to vertical winds
      ...
      real(kind=kind_real)    :: tau = 0.   !< Time scale (in days) for Rayleigh friction applied to horizontal
   ...
   !------------------------------------------------
      ...
      logical :: nudge_qv  = .false.   !< During the adiabatic initialization (na_init > 0), if set to .true.,
                                       ...
                                       !< The default is .false.
      real(kind=kind_real)    :: add_noise = -1.  !< Amplitude of random thermal noise (in K) to add upon startup.
      ...
      real(kind=kind_real) :: dz_min = 2        !< Minimum thickness depth to  to enforce monotonicity of height to prevent blowup.
                                !< 2 by default
     ...
     !real(kind=kind_real),    pointer :: alpha
     ...
     type fv_nest_BC_type_3D
        ...
        real(kind=kind_real), allocatable, dimension(:,:,:) :: west_t1, east_t1, south_t1, north_t1
        real(kind=kind_real), allocatable, dimension(:,:,:) :: west_t0, east_t0, south_t0, north_t0
   ...
   !nested grid flags:
        ...
        real(kind=kind_real)    :: s_weight = 1.e-6 !<sponge weight
        ...
        real(kind=kind_real)    :: update_blend = 1. !< option for controlling how much "blending" is done during two-way update
     ...
     type inline_mp_type
       real(kind=kind_real), _ALLOCATABLE :: prer(:,:)     _NULL
       real(kind=kind_real), _ALLOCATABLE :: prei(:,:)     _NULL
       real(kind=kind_real), _ALLOCATABLE :: pres(:,:)     _NULL
       real(kind=kind_real), _ALLOCATABLE :: preg(:,:)     _NULL

       real(kind=kind_real), _ALLOCATABLE :: qv_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: ql_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: qi_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: liq_wat_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: ice_wat_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: qr_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: qg_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: qs_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: t_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: u_dt(:,:,:)
       real(kind=kind_real), _ALLOCATABLE :: v_dt(:,:,:)
     end type inline_mp_type

     type phys_diag_type

        real(kind=kind_real), _ALLOCATABLE :: phys_t_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_qv_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_ql_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_qi_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_liq_wat_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_ice_wat_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_qr_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_qg_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_qs_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_u_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phys_v_dt(:,:,:)

     end type phys_diag_type

     type nudge_diag_type

        real(kind=kind_real), _ALLOCATABLE :: nudge_t_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: nudge_ps_dt(:,:)
        real(kind=kind_real), _ALLOCATABLE :: nudge_delp_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: nudge_u_dt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: nudge_v_dt(:,:,:)

     end type nudge_diag_type

     type coarse_restart_type

        real(kind=kind_real), _ALLOCATABLE :: u(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: v(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: w(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: pt(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: q(:,:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: qdiag(:,:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: delz(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: phis(:,:)
        real(kind=kind_real), _ALLOCATABLE :: delp(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: ua(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: va(:,:,:)
        real(kind=kind_real), _ALLOCATABLE :: u_srf(:,:)
        real(kind=kind_real), _ALLOCATABLE :: v_srf(:,:)
        real(kind=kind_real), _ALLOCATABLE :: sgh(:,:)
        real(kind=kind_real), _ALLOCATABLE :: oro(:,:)
        real(kind=kind_real), _ALLOCATABLE :: ze0(:,:,:)
   ...
   ! from the D grid variables.
       real(kind=kind_real), _ALLOCATABLE :: u(:,:,:)    _NULL  !< D grid zonal wind (m/s)
       real(kind=kind_real), _ALLOCATABLE :: v(:,:,:)    _NULL  !< D grid meridional wind (m/s)
       real(kind=kind_real), _ALLOCATABLE :: pt(:,:,:)   _NULL  !< temperature (K)
       real(kind=kind_real), _ALLOCATABLE :: delp(:,:,:) _NULL  !< pressure thickness (pascal)
       real(kind=kind_real), _ALLOCATABLE :: q(:,:,:,:)  _NULL  !< specific humidity and prognostic constituents
       real(kind=kind_real), _ALLOCATABLE :: qdiag(:,:,:,:)  _NULL  !< diagnostic tracers

   !----------------------
   ! non-hydrostatic state:
   !----------------------------------------------------------------------
       real(kind=kind_real), _ALLOCATABLE ::     w(:,:,:)  _NULL  !< cell center vertical wind (m/s)
       real(kind=kind_real), _ALLOCATABLE ::  delz(:,:,:)  _NULL  !< layer thickness (meters)
       real(kind=kind_real), _ALLOCATABLE ::   ze0(:,:,:)  _NULL  !< height at layer edges for remapping
       real(kind=kind_real), _ALLOCATABLE ::  q_con(:,:,:) _NULL  !< total condensates
   ...
   !-----------------------------------------------------------------------
   ! dyn_aux:
       real(kind=kind_real), _ALLOCATABLE :: ps (:,:)      _NULL  !< Surface pressure (pascal)
       real(kind=kind_real), _ALLOCATABLE :: pe (:,:,: )   _NULL  !< edge pressure (pascal)
       real(kind=kind_real), _ALLOCATABLE :: pk  (:,:,:)   _NULL  !< pe**cappa
       real(kind=kind_real), _ALLOCATABLE :: peln(:,:,:)   _NULL  !< ln(pe)
   ...
   ! For phys coupling:
       ...
       real(kind=kind_real), _ALLOCATABLE :: v_srf(:,:)    _NULL  !< Surface v-wind
       real(kind=kind_real), _ALLOCATABLE :: sgh(:,:)      _NULL  !< Terrain standard deviation
       real(kind=kind_real), _ALLOCATABLE :: oro(:,:)      _NULL  !< land fraction (1: all land; 0: all water)
       real(kind=kind_real), _ALLOCATABLE :: ts(:,:)       _NULL  !< skin temperature (sst) from NCEP/GFS (K) -- tile

   ! For stochastic kinetic energy backscatter (SKEB)
       real(kind=kind_real), _ALLOCATABLE :: diss_est(:,:,:) _NULL !< dissipation estimate taken from 'heat_source'

   !-----------------------------------------------------------------------
   ! Others:
   !-----------------------------------------------------------------------
       real(kind=kind_real), _ALLOCATABLE :: phis(:,:)     _NULL  !< Surface geopotential (g*Z_surf)
       real(kind=kind_real), _ALLOCATABLE :: omga(:,:,:)   _NULL  !< Vertical pressure velocity (pa/s)
       real(kind=kind_real), _ALLOCATABLE :: ua(:,:,:)     _NULL  !< (ua, va) are mostly used as the A grid winds
       real(kind=kind_real), _ALLOCATABLE :: va(:,:,:)     _NULL
       real(kind=kind_real), _ALLOCATABLE :: uc(:,:,:)     _NULL  !< (uc, vc) are mostly used as the C grid winds
       real(kind=kind_real), _ALLOCATABLE :: vc(:,:,:)     _NULL

       real(kind=kind_real), _ALLOCATABLE :: ak(:)  _NULL
       real(kind=kind_real), _ALLOCATABLE :: bk(:)  _NULL

      integer :: ks

   ! Accumulated Mass flux arrays
       real(kind=kind_real), _ALLOCATABLE ::  mfx(:,:,:)  _NULL
       real(kind=kind_real), _ALLOCATABLE ::  mfy(:,:,:)  _NULL
   ! Accumulated Courant number arrays
       real(kind=kind_real), _ALLOCATABLE ::  cx(:,:,:)  _NULL
       real(kind=kind_real), _ALLOCATABLE ::  cy(:,:,:)  _NULL

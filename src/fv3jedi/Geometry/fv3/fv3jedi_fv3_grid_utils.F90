...
use fv3jedi_kinds_mod,          only: kind_real
...
#endif
    real(kind=kind_real), parameter::  big_number=1.d8
    real(kind=kind_real), parameter:: tiny_number=1.d-8

    real(kind=R_GRID) :: radius=cnst_radius

    real(kind=kind_real), parameter:: ptop_min=1.d-8
   ...
   !     For MOLECULAR_DIFFUSION
         ...
         real(kind=kind_real), pointer, dimension(:,:) :: sina_6
         real(kind=kind_real), pointer, dimension(:,:) :: delu_6, delv_6, delu_5, delv_5

         real(kind=kind_real), pointer, dimension(:,:) :: del6_u, del6_v
         real(kind=kind_real), pointer, dimension(:,:) :: divg_u, divg_v
         real(kind=kind_real), pointer, dimension(:,:) :: cosa_u, cosa_v, cosa_s
         real(kind=kind_real), pointer, dimension(:,:) :: sina_u, sina_v
         real(kind=kind_real), pointer, dimension(:,:) :: rsin_u, rsin_v
         real(kind=kind_real), pointer, dimension(:,:) :: rsina, rsin2
         real(kind=kind_real), pointer, dimension(:,:,:) :: sin_sg, cos_sg
   ...
   !
     ...
     real(kind=kind_real) function inner_prod(v1, v2)
   ...
   !-------------------------------------
    ...
    real(kind=kind_real) function great_circle_dist( q1, q2, radius )
    ...
    subroutine intersect_cross(a1,a2,b1,b2,radius,x_inter,local_a,local_b)
     ...
     !Local pointers
     real(kind=kind_real), pointer, dimension(:,:) :: a11, a12, a21, a22
     real(kind=kind_real), pointer, dimension(:,:) :: z11, z12, z21, z22
   ...
   !          if(.not.hydrostatic) gridstruct%w00(i,j) = 2.d0*omega*cos(agrid(i,j,2))
    ...
    real(kind=kind_real), intent(inout):: u(bd%isd:bd%ied,bd%jsd:bd%jed+1,km)
    real(kind=kind_real), intent(inout):: v(bd%isd:bd%ied+1,bd%jsd:bd%jed,km)
    real(kind=kind_real), intent(out):: ua(bd%isd:bd%ied, bd%jsd:bd%jed,km)
    real(kind=kind_real), intent(out):: va(bd%isd:bd%ied, bd%jsd:bd%jed,km)
   ...
   ! 4-pt Lagrange interpolation
     real(kind=kind_real) :: a1 =  0.5625
     real(kind=kind_real) :: a2 = -0.0625
     real(kind=kind_real) :: c1 =  1.125
     real(kind=kind_real) :: c2 = -0.125
     real(kind=kind_real) utmp(bd%is:bd%ie,  bd%js:bd%je+1)
     real(kind=kind_real) vtmp(bd%is:bd%ie+1,bd%js:bd%je)
     real(kind=kind_real) wu(bd%is:bd%ie,  bd%js:bd%je+1)
     real(kind=kind_real) wv(bd%is:bd%ie+1,bd%js:bd%je)
     integer i, j, k
   ...
   ! Simple Cartesian Geometry:
    ...
    type(fv_grid_bounds_type), intent(IN) :: bd
     ...
     real(kind=kind_real), intent(in) ::  u(bd%isd:bd%ied,bd%jsd:bd%jed+1,km)
     real(kind=kind_real), intent(in) ::  v(bd%isd:bd%ied+1,bd%jsd:bd%jed,km)
    type(fv_grid_type), intent(IN), target :: gridstruct
   ...
   !--------------------------------------------------------------
   ! Local
     real(kind=kind_real) wu(bd%is-1:bd%ie+1,  bd%js-1:bd%je+2)
     real(kind=kind_real) wv(bd%is-1:bd%ie+2,  bd%js-1:bd%je+1)
     real(kind=kind_real) u1(bd%is-1:bd%ie+1), v1(bd%is-1:bd%ie+1)
     integer i, j, k
     integer :: is,  ie,  js,  je

     real(kind=kind_real), dimension(:,:), pointer :: a11, a12, a21, a22
     real(kind=kind_real), dimension(:,:), pointer :: dx, dy, rdxa, rdya
   ...
   !>@brief The function 'global_qsum' computes the quick global sum without area weighting.
    real(kind=kind_real) function global_qsum(p, ifirst, ilast, jfirst, jlast)
         integer, intent(IN) :: ifirst, ilast
   ...
   !>@brief The subroutine 'global_mx_c' computes the global max and min at the cell corners.
    ...
    type(fv_grid_bounds_type), intent(IN) :: bd
     ...
     integer, intent(in):: npx, npy
     real(kind=kind_real), intent(in):: value
     integer i,j
    ...
    type(fv_grid_bounds_type), intent(IN) :: bd
     ...
     real(kind=kind_real), intent(INOUT) :: ptop
     real(kind=kind_real), intent(inout):: pe(bd%is-1:bd%ie+1,km+1,bd%js-1:bd%je+1)
     real(kind=kind_real), intent(out):: ak(km+1), bk(km+1)
     type(domain2d), intent(IN) :: domain
   ! local:
     real(kind=kind_real) ph(km+1)
     real(kind=kind_real), allocatable:: pem(:,:)
     real(kind=4) :: p4
     integer k, i, j
   ...
   ! Purpose; Transform wind tendencies on A grid to D grid for the final update
     ...
     real(kind=kind_real), intent(inout):: u(isd:ied,  jsd:jed+1,npz)
     real(kind=kind_real), intent(inout):: v(isd:ied+1,jsd:jed  ,npz)
     real(kind=kind_real), intent(inout), dimension(isd:ied,jsd:jed,npz):: u_dt, v_dt
     type(fv_grid_type), intent(IN), target :: gridstruct
     type(domain2d), intent(INOUT) :: domain

   ! local:
     real(kind=kind_real) v3(is-1:ie+1,js-1:je+1,3)
     real(kind=kind_real) ue(is-1:ie+1,js:je+1,3)    ! 3D winds at edges
     real(kind=kind_real) ve(is:ie+1,js-1:je+1,  3)    ! 3D winds at edges
     real(kind=kind_real), dimension(is:ie):: ut1, ut2, ut3
     real(kind=kind_real), dimension(js:je):: vt1, vt2, vt3
     real(kind=kind_real) dt5, gratio
   ...
   ! local:
     real(kind=kind_real) ut(isd:ied,jsd:jed)
     real:: dt5, gratio
     integer i, j, k
     ...
     real(kind=R_GRID), pointer, dimension(:,:,:,:) :: es, ew
     real(kind=R_GRID), pointer, dimension(:) :: edge_vect_w, edge_vect_e, edge_vect_s, edge_vect_n
     real(kind=kind_real), pointer, dimension(:,:) :: z11, z12, z21, z22, dya, dxa
   ...
   ! East Edges:
    ...
    subroutine init_mq(phis, gridstruct, npx, npy, is, ie, js, je, ng)
       ...
       real(kind=kind_real), intent(in):: phis(is-ng:ie+ng, js-ng:je+ng)
       type(fv_grid_type), intent(IN), target :: gridstruct

   ! local:
       real(kind=kind_real) zs(is-ng:ie+ng, js-ng:je+ng)
       real(kind=kind_real) zb(is-ng:ie+ng, js-ng:je+ng)
       real(kind=kind_real) pdx(3,is:ie,js:je+1)
       real(kind=kind_real) pdy(3,is:ie+1,js:je)
       integer i, j, n

       real(kind=kind_real), pointer :: rarea(:,:)
       real(kind=kind_real), pointer, dimension(:,:) :: dx, dy
       real(kind=kind_real), pointer, dimension(:,:,:) :: en1, en2, agrid, vlon, vlat

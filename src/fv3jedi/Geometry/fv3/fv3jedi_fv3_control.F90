...
use fv3jedi_kinds_mod,          only: kind_real
...
#ifdef OVERLOAD_R4
      real(kind=kind_real)    :: too_big  = 1.E8
#else
      real(kind=kind_real)    :: too_big  = 1.E35
#endif
      ...
      real(kind=kind_real)    :: alpha = 0.0
      integer :: Nsolitons = 1
      real(kind=kind_real)    :: soliton_size = 750.e3, soliton_Umax = 50.
...
#include<file_version.h>
   ...
   !-------------------------------------------------------------------------------
      ...
      subroutine fv_control_init(Atm, dt_atmos, this_grid, grids_on_this_pe, p_split, &
        ...
        real(kind=kind_real),                intent(in)    :: dt_atmos
        ...
        real(kind=kind_real)    , pointer :: scale_z
        real(kind=kind_real)    , pointer :: w_max
        real(kind=kind_real)    , pointer :: z_min
        real(kind=kind_real)    , pointer :: lim_fac

        integer , pointer :: nord
        integer , pointer :: nord_tr
        real(kind=kind_real)    , pointer :: dddmp
        real(kind=kind_real)    , pointer :: d2_bg
        real(kind=kind_real)    , pointer :: d4_bg
        real(kind=kind_real)    , pointer :: vtdm4
        real(kind=kind_real)    , pointer :: trdm2
        real(kind=kind_real)    , pointer :: d2_bg_k1
        real(kind=kind_real)    , pointer :: d2_bg_k2
        real(kind=kind_real)    , pointer :: d2_divg_max_k1
        real(kind=kind_real)    , pointer :: d2_divg_max_k2
        real(kind=kind_real)    , pointer :: damp_k_k1
        real(kind=kind_real)    , pointer :: damp_k_k2
        ...
        ! PG off centering:
        real(kind=kind_real)    , pointer :: beta
        integer , pointer :: n_sponge
        real(kind=kind_real)    , pointer :: d_ext
        integer , pointer :: nwat
        logical , pointer :: warm_start
        logical , pointer :: inline_q
        real(kind=kind_real) , pointer :: shift_fac
        ...
        real(kind=kind_real)    , pointer :: p_fac
        real(kind=kind_real)    , pointer :: a_imp
        integer , pointer :: n_split
        real(kind=kind_real)    , pointer :: fac_n_spl
        real(kind=kind_real)    , pointer :: fhouri
        ! Default
        ...
        real(kind=kind_real)    , pointer :: sg_cutoff

        integer , pointer :: na_init
        logical , pointer :: nudge_dz
        real(kind=kind_real)    , pointer :: p_ref
        real(kind=kind_real)    , pointer :: dry_mass
        integer , pointer :: nt_prog
        integer , pointer :: nt_phys
        real(kind=kind_real)    , pointer :: tau_h2o

        real(kind=kind_real)    , pointer :: delt_max
        real(kind=kind_real)    , pointer :: d_con
        real(kind=kind_real)    , pointer :: ke_bg
        real(kind=kind_real)    , pointer :: consv_te
        real(kind=kind_real)    , pointer :: tau
        real(kind=kind_real)    , pointer :: tau_w
        real(kind=kind_real)    , pointer :: fast_tau_w_sec
        real(kind=kind_real)    , pointer :: rf_cutoff
      ...
      logical , pointer :: hrrrv3_ic
        ...
        real(kind=kind_real),     pointer :: add_noise
        logical , pointer :: butterfly_effect
        logical , pointer :: molecular_diffusion
        real(kind=kind_real),     pointer :: dz_min
        ...
        real(kind=kind_real), pointer :: s_weight, update_blend
   ...
   !! can be read in from the namelist.
        ...
        subroutine read_namelist_fv_core_nml(Atm)
          ...
          integer :: f_unit, ios, ierr
          real(kind=kind_real) :: dim0 = 180.           ! base dimension
          real(kind=kind_real) :: dt0  = 1800.          ! base time step
          real(kind=kind_real) :: ns0  = 5.             ! base nsplit for base dimension
          real(kind=kind_real) :: dimx, dl, dp, dxmin, dymin, d_fac
          real(kind=kind_real) :: umax = 350.           ! max wave speed for grid_type>3

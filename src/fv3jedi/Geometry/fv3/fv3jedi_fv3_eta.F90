...
! This is the easy to use version of the set_eta
      ...
      real(kind=kind_real):: a60(61),b60(61)
...
! 3 layers
      ...
      real(kind=kind_real), intent(out):: ak(km+1)
      real(kind=kind_real), intent(out):: bk(km+1)
      real(kind=kind_real), intent(out):: ptop         ! model top (Pa)
      character(24), intent(IN) :: npz_type
      real(kind=kind_real) pint, stretch_fac
      integer  k
      real(kind=kind_real) :: s_rate = -1.0 ! dummy value to not use var_les
...
! Equivalent Shallow Water: for NGGPS, variable-resolution testing
            ...
            ks = 0
            do k=1,km+1
               bk(k) = real(k-1,kind=kind_real) / real (km,kind=kind_real)
               ak(k) = ptop*(1.-bk(k))
            enddo
         else
#ifndef GFSL60
...
#define _FV_ETA__
   ...
   ! local
         real(kind=kind_real) a24(25),b24(25)            ! GFDL AM2L24
         real(kind=kind_real) a26(27),b26(27)            ! Jablonowski & Williamson 26-level
         real(kind=kind_real) a32old(33),b32old(33)
         real(kind=kind_real) a32(33),b32(33)
         real(kind=kind_real) a32w(33),b32w(33)
         real(kind=kind_real) a33(34),b33(34)             ! miz: grid with enhanced surface-layer resolution
         real(kind=kind_real) a47(48),b47(48)
         real(kind=kind_real) a48(49),b48(49)
         real(kind=kind_real) a50(51),b50(51)            ! kyc: HRRRv3 grid
         real(kind=kind_real) a52(53),b52(53)
         real(kind=kind_real) a54(55),b54(55)
         real(kind=kind_real) a56(57),b56(57)
         real(kind=kind_real) a60(61),b60(61)
         real(kind=kind_real) a60gfs(61),b60gfs(61)
         real(kind=kind_real) a63(64),b63(64)
         real(kind=kind_real) a63meso(64),b63meso(64)
         real(kind=kind_real) a64(65),b64(65)
         real(kind=kind_real) a64gfs(65),b64gfs(65)
         real(kind=kind_real) a68(69),b68(69)            ! cjg: grid with enhanced PBL resolution
         real(kind=kind_real) a96(97),b96(97)            ! cjg: grid with enhanced PBL resolution
         real(kind=kind_real) a100(101),b100(101)
         real(kind=kind_real) a104(105),b104(105)
         real(kind=kind_real) a125(126),b125(126)
   ...
   real(kind=kind_real):: p0=1000.E2
   real(kind=kind_real):: pc=200.E2

   real(kind=kind_real) pt, lnpe, dlnp
   real(kind=kind_real) press(km+1), pt1(km)
   integer :: l, k
   integer :: var_fn = 0

   real(kind=kind_real) :: pint = 100.E2
   real(kind=kind_real) :: stretch_fac = 1.03
...
!! 'ak' and 'bk'
 subroutine set_external_eta(ak, bk, ptop, ks)
   implicit none
   real(kind=kind_real),    intent(in)  :: ak(:)
   real(kind=kind_real),    intent(in)  :: bk(:)
   real(kind=kind_real),    intent(out) :: ptop         !< model top (Pa)
   integer, intent(out) :: ks           !< number of pure p layers
   !--- local variables
   integer :: k
   real(kind=kind_real) :: eps = 1.d-7
 ...
 implicit none
  ...
  real(kind=kind_real),    intent(in):: s_rate        !< between [1. 1.1]
  real(kind=kind_real),    intent(out):: ak(km+1), bk(km+1)
  real(kind=kind_real),    intent(inout):: pint
  integer, intent(out):: ks
! Local
  real(kind=kind_real), parameter:: p00 = 1.E5
  real(kind=kind_real), dimension(km+1):: ze, pe1, peln, eta
  real(kind=kind_real), dimension(km):: dz, s_fac, dlnp, pm, dp, dk
  real(kind=kind_real) ztop, t0, dz0, sum1, tmp1
  real(kind=kind_real) ep, es, alpha, beta, gama
  real(kind=kind_real), parameter:: akap = 2./7.
!---- Tunable parameters:
  integer:: k_inc = 10   !< number of layers from bottom up to near const dz region
  real(kind=kind_real):: s0 = 0.8     !< lowest layer stretch factor
!-----------------------
  real(kind=kind_real):: s_inc
  integer  k
     ...
     ztop = rdgas/grav*t0*(peln(km+1) - peln(1))
      ...
      s_inc = (1.-s0) / real(k_inc,kind=kind_real)
      s_fac(km)  = s0
...
! Local
  real(kind=kind_real), parameter:: p00 = 1.E5
  real(kind=kind_real), dimension(km+1):: ze, pe1, peln, eta
  real(kind=kind_real), dimension(km):: dz, s_fac, dlnp
  real(kind=kind_real) ztop, t0, dz0, sum1, tmp1
  real(kind=kind_real) ep, es, alpha, beta, gama
!---- Tunable parameters:
  integer:: k_inc = 25   !< number of layers from bottom up to near const dz region
  real(kind=kind_real):: s0 = 0.13 !< lowest layer stretch factor
!-----------------------
  real(kind=kind_real):: s_inc
  integer  k
...
!---- Tunable parameters:
  ...
  real(kind=kind_real):: s0 = 0.10 !< lowest layer stretch factor
!-----------------------
  real(kind=kind_real):: s_inc
  integer  k
...
! Pure pressure:
 ...
 subroutine hybrid_z_dz(km, dz, ztop, s_rate)
  integer, intent(in):: km
  real(kind=kind_real),    intent(in):: s_rate        !< between [1. 1.1]
  real(kind=kind_real),    intent(in):: ztop
  real(kind=kind_real),    intent(out):: dz(km)
! Local
  real(kind=kind_real), parameter:: p00 = 1.E5
  real(kind=kind_real), dimension(km+1):: ze, pe1, peln, eta
  real(kind=kind_real), dimension(km):: s_fac, dlnp
  real(kind=kind_real) t0, dz0, sum1, tmp1
  real(kind=kind_real) ep, es, alpha, beta, gama
  integer  k
...
!! layer-mean pressures for reference.
 subroutine get_eta_level(npz, p_s, pf, ph, ak, bk, pscale)
  ...
  real(kind=kind_real), intent(in)  :: p_s            !< unit: pascal
  real(kind=kind_real), intent(in)  :: ak(npz+1)
  real(kind=kind_real), intent(in)  :: bk(npz+1)
  real(kind=kind_real), intent(in), optional :: pscale
  real(kind=kind_real), intent(out) :: pf(npz)
  real(kind=kind_real), intent(out) :: ph(npz+1)
  integer k
 ...
 subroutine compute_dz(km, ztop, dz)
  ...
  integer, intent(in):: km
  real(kind=kind_real),   intent(in):: ztop        ! try 50.E3
  real(kind=kind_real),   intent(out):: dz(km)
!------------------------------
  real(kind=kind_real) ze(km+1), dzt(km)
  integer k


! ztop = 30.E3
  dz(1) = ztop / real(km,kind=kind_real)
  dz(km) = 0.5*dz(1)
...
! Top:
 ...
 subroutine compute_dz_var(km, ztop, dz)
  ...
  real(kind=kind_real),   intent(out):: dz(km)
!------------------------------
  real(kind=kind_real), parameter:: s_rate = 1.0
  real(kind=kind_real) ze(km+1)
  real(kind=kind_real) s_fac(km)
  real(kind=kind_real) sum1, dz0
  integer k
...
! Re-scale dz with the stretched ztop
 ...
 subroutine compute_dz_L32(km, ztop, dz)
  ...
  integer, intent(in):: km
  real(kind=kind_real),   intent(out):: dz(km)
  real(kind=kind_real),   intent(out):: ztop        ! try 50.E3
!------------------------------
  real(kind=kind_real) dzt(km)
  real(kind=kind_real) ze(km+1)
  real(kind=kind_real) dz0, dz1, dz2
  real(kind=kind_real) z0, z1, z2
...
!       endif
 ...
 subroutine compute_dz_L101(km, ztop, dz)
  ...
  real(kind=kind_real),   intent(out):: dz(km)
  real(kind=kind_real),   intent(out):: ztop      ! try 30.E3
!------------------------------
  real(kind=kind_real) ze(km+1)
  real(kind=kind_real) dz0, dz1
  real(kind=kind_real):: stretch_f = 1.16
  integer k, k0, k1
...
!  ztop (km) = 20.2859154
 ...
 real(kind=kind_real), intent(in):: rgrav, ztop
 real(kind=kind_real), intent(in):: dz(km)       !< Reference vertical resolution for zs=0
 real(kind=kind_real), intent(in):: hs(is-ng:ie+ng,js-ng:je+ng)
 real(kind=kind_real), intent(inout)::  ze(is:ie,js:je,km+1)
 real(kind=kind_real), optional, intent(out):: dz3(is-ng:ie+ng,js-ng:je+ng,km)
! local
 logical:: filter_xy = .false.
 real(kind=kind_real), allocatable:: delz(:,:,:)
 integer ntimes
 real(kind=kind_real) zint
 real(kind=kind_real):: z1(is:ie,js:je)
 real(kind=kind_real):: z(km+1)
 real(kind=kind_real) sig, z_rat
...
!--------------------------------------
  ...
  real(kind=kind_real), intent(inout):: ze(is:ie,js:je,km+1)
! local:
  real(kind=kind_real), parameter:: df = 0.25
  real(kind=kind_real) dz(km)
  real(kind=kind_real) flux(km+1)
  integer k, n, k1, k2
  ...
  integer, intent(in):: km
  real(kind=kind_real),    intent(in):: p0, ztop
  real(kind=kind_real),    intent(inout):: ptop
  real(kind=kind_real),    intent(inout):: ak(km+1), bk(km+1)
  real(kind=kind_real), intent(out):: pt1(km)
! Local
  logical:: isothermal
  real(kind=kind_real), dimension(km+1):: ze, pe1, pk1
  real(kind=kind_real), dimension(km):: dz1
  real(kind=kind_real) t0, n2, s0
  integer  k
...
! Set up vertical coordinare with constant del-z spacing:
       ...
       ze(km+1) = 0.
       do k=km,1,-1
          dz1(k) = ztop / real(km,kind=kind_real)
           ze(k) = ze(k+1) + dz1(k)
       enddo
...
! Compute volume mean potential temperature with hydrostatic eqn:
 ...
 subroutine mount_waves(km, ak, bk, ptop, ks, pint)
  integer, intent(in):: km
  real(kind=kind_real),    intent(out):: ak(km+1), bk(km+1)
  real(kind=kind_real),    intent(out):: ptop, pint
  integer, intent(out):: ks
! Local
  real(kind=kind_real), parameter:: p00 = 1.E5
  real(kind=kind_real), dimension(km+1):: ze, pe1, peln, eta
  real(kind=kind_real), dimension(km):: dz, dlnp
  real(kind=kind_real) ztop, t0, dz0, sum1, tmp1
  real(kind=kind_real) ep, es, alpha, beta, gama, s_fac
  integer  k, k500
...
! ztop = real(km-1,kind=kind_real)*500.
!-----------------------
...
! Pure pressure:
  ...
  real(kind=kind_real), intent(inout):: q(im,km)
!---
  integer i, k
  real(kind=kind_real) qtmp

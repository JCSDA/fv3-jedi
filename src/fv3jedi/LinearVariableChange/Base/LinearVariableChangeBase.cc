...
LinearVariableChangeFactory::LinearVariableChangeFactory(const std::string & name) {
  if (getMakers().find(name) != getMakers().end()) {
    oops::Log::error() << name << " already registered in fv3jedi::LinearVariableChangeFactory."
                       << std::endl;
    ABORT("Element already registered in fv3jedi::LinearVariableChangeFactory.");
  }
  getMakers()[name] = this;
}
...
LinearVariableChangeBase * LinearVariableChangeFactory::create(const State & xbg,
  ...
  if (jloc == getMakers().end()) {
    oops::Log::error() << id << " does not exist in fv3jedi::LinearVariableChangeFactory."
                       << std::endl;
    ABORT("Element does not exist in fv3jedi::LinearVariableChangeFactory.");
  }
...
LinearVariableChangeFactory::createParameters(const std::string &name) {
  typename std::map<std::string, LinearVariableChangeFactory*>::iterator it =
      getMakers().find(name);
  if (it == getMakers().end()) {
    throw std::runtime_error(name + " does not exist in fv3jedi::LinearVariableChangeFactory");
  }

...
// -------------------------------------------------------------------------------------------------
IOGeos::IOGeos(const Geometry & geom, const Parameters_ & params) : IOBase(geom) {
  util::Timer timer(classname(), "IOGeos");
  oops::Log::trace() << classname() << " constructor starting" << std::endl;
  fv3jedi_io_geos_create_f90(objectKeyForFortran_, params.toConfiguration(), geom.toFortran());

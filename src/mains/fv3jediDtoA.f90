...
! Read the restarts
! -----------------
call read_d(config_input, geom, u, v)

! Convert the winds
! -----------------
call d_to_a(geom, u, v, ua, va)

! Write the restarts
! ------------------
call write_a(config_output, geom, ua, va)
...
if (config%has("testing")) then
  ...
  ! Check output
  call testing(comm, geom, config_test, ua, va)
...
subroutine read_d(conf, geom, u, v)
...
if ( open_file(rst, trim(dpath)//'/'//trim(fcore), "read", geom%domain, is_restart=.true., &
   ...
   call abor1_ftn("fv3jediDtoA: failed to open file for reading: "//trim(dpath)//'/'//trim(fcore))
end if
...
subroutine write_a(conf, geom, ua, va)

! Arguments
type(fckit_configuration), intent(in) :: conf
type(fv3jedi_geom),        intent(in) :: geom
real(kind=kind_real),      intent(in) :: ua(geom%isc:geom%iec,geom%jsc:geom%jec,1:geom%npz)
real(kind=kind_real),      intent(in) :: va(geom%isc:geom%iec,geom%jsc:geom%jec,1:geom%npz)
...
if ( open_file(rst, trim(dpath)//'/'//trim(fcore), "overwrite", geom%domain, is_restart=.true., &
               dont_add_res_to_filename=.true.) ) then
   call fv3jedi_register_field(rst, 'ua', ua, center, .true., &
                               'eastward_wind', 'ms-1')
   call fv3jedi_register_field(rst, 'va', va, center, .true., &
                               'northward_wind', 'ms-1')

   call write_restart(rst)
   call close_file(rst)
else
   call abor1_ftn("fv3jediDtoA: failed to open file for writing: "//trim(dpath)//'/'//trim(fcore))
end if
...
subroutine testing(comm, geom, conf, ua, va)
...
tmp(1) = minval(ua(isc:iec,jsc:jec,1:npz), mask=ua(isc:iec,jsc:jec,1:npz)<10e10)
tmp(2) = maxval(ua(isc:iec,jsc:jec,1:npz), mask=ua(isc:iec,jsc:jec,1:npz)<10e10)
tmp(3) =    sum(ua(isc:iec,jsc:jec,1:npz)**2, mask=u(isc:iec,jsc:jec,1:npz)<10e10)
...
tmp(1) = minval(va(isc:iec,jsc:jec,1:npz), mask=va(isc:iec,jsc:jec,1:npz)<10e10)
tmp(2) = maxval(va(isc:iec,jsc:jec,1:npz), mask=va(isc:iec,jsc:jec,1:npz)<10e10)
tmp(3) =    sum(va(isc:iec,jsc:jec,1:npz)**2, mask=va(isc:iec,jsc:jec,1:npz)<10e10)

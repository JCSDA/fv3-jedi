...
do n = 1,size(fields)
  ...
  do lev = 1,fields(n)%npz
    ...
    if (f_comm%size() > 6) then
      call scatter_tile(geom, commtile, 1, arrayg, fields(n)%array(geom%isc:geom%iec,geom%jsc:geom%jec,lev))
    else
...
if (f_comm%size() > 6) then
  call gather_tile(geom, commtile, 1, rad2deg*geom%grid_lon(geom%isc:geom%iec,geom%jsc:geom%jec), arrayg)
else
...
if (f_comm%size() > 6) then
  call gather_tile(geom, commtile, 1, rad2deg*geom%grid_lat(geom%isc:geom%iec,geom%jsc:geom%jec), arrayg)
else
...
do n = 1,size(fields)
  ...
  do lev = 1,fields(n)%npz
    ...
    if (f_comm%size() > 6) then
      call gather_tile(geom, commtile, 1, fields(n)%array(geom%isc:geom%iec,geom%jsc:geom%jec,lev), arrayg)
    else
